...
/**
 * Taskassignments represents the model behind the search form of `frontend\models\Taskassignment`.
 */
class Taskassignments extends Taskassignment
{
    ...
    public function searchAssigned($params)
    {
        ...
        $id = Worker::find()->where(['applicantID' => $applicant->ApplicantID])->one();
        if($id){
            $query = Taskassignment::find()
